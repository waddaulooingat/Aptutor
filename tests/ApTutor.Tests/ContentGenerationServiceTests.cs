...
public class ContentGenerationServiceTests
{
    ...
    public void ClearJob_TwoConcurrentCallers_OnlyOneSucceeds()
    {
        ...
        service.RestoreJob(new GenerationJob("csa", "u1.1", Difficulty.Medium, false, GenerationStatus.Succeeded, null, SamplePack("u1.1"), DateTimeOffset.UtcNow));
    ...
    {
        var service = NewService();
        service.RestoreJob(new GenerationJob("csa", "u1.1", Difficulty.Medium, false, GenerationStatus.Failed, "boom", null, DateTimeOffset.UtcNow));
    ...
    {
        var service = NewService();
        var older = new GenerationJob("csa", "u1.1", Difficulty.Medium, false, GenerationStatus.Succeeded, null, SamplePack("u1.1"), DateTimeOffset.UtcNow.AddMinutes(-5));
        var newer = new GenerationJob("csa", "u1.1", Difficulty.Medium, false, GenerationStatus.Running, null, null, DateTimeOffset.UtcNow);
    ...
    public void TryStartGeneration_RefusesADuplicateWhileAJobIsAlreadyTracked()
    {
        ...
        var started = service.TryStartGeneration("csa", "u1.1", Difficulty.Medium, allowGraphChoices: false);
