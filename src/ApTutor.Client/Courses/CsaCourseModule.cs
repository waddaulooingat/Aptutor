...
{
    // This fixture bank predates graph-based choices (see the graph-spec-rendering plan) — every
    // option here is always plain text, so wrapping each one is a one-line mechanical adaptation to
    // PracticeItem's new Choices shape, not a behavior change.
    ...
    private static readonly IReadOnlyList<PracticeItem> Items = new List<PracticeItem>
    {
        new("u1.1-q1", "u1.1", "Which best describes what happens when you run a compiled Java program?",
            Txt("The .java source is re-parsed line by line as it executes",
        ...
        new("u1.1-q2", "u1.1", "What is the required name of the file containing a public class Main?",
            Txt("main.java", "Main.txt", "Main.java", "any name ending in .class"),
        ...
        new("u1.2-q1", "u1.2", "Which declaration is valid Java?",
            Txt("int x = 3.5;", "boolean flag = 1;", "double d = 7;", "int 2nd = 5;"),
        ...
        new("u2.1-q1", "u2.1", "After Point p = new Point(); Point q = p; q.x = 9; what is p.x?",
            Txt("0 (unchanged)", "9", "A compile error", "null"),
        ...
        new("u2.2-q1", "u2.2", "What does `Point p;` (a local variable, never assigned) contain before first use?",
            Txt("null, ready to use", "A default Point object", "Nothing usable — the compiler rejects reading it unassigned", "0"),
        ...
        new("u2.2-q2", "u2.2", "What is `p` after `Point p = new Point();` if Point has no declared constructor?",
            Txt("A compile error — Point needs an explicit constructor", "A reference to a new Point built by the compiler-supplied no-arg constructor", "null", "An uninitialized Point"),
